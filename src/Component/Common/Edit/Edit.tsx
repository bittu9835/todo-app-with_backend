...
const Edit: FC<EditProps> = () => {
    ...
    const fetchData = async () => {
        axios.get(`http://localhost:4000/api/notes/${editData._id}/getForEdit`, { headers })
            .then((response) => {
    ...
    const handleCheck = (todo: any) => {
        if (todo.isCompleted === true) {
            axios.delete(`http://localhost:4000/api/notes/${editData._id}/CkeckoutNotes`, { headers })
                .then((response) => {
                    setRender(!rander)
            ...
            axios.delete(`http://localhost:4000/api/notes/${editData._id}/completed`, { headers })
                .then((response) => {
                    setRender(!rander)
                    toast.success(response.data.message)
                })
    ...
    const handleDelete = () => {
        axios.delete(`http://localhost:4000/api/notes/${editData._id}/delete`, { headers })
            .then((response) => {
                // fetchData()
                setRender(!rander)
                toast.success(response.data.message)
    ...
    const handleUpdate = (values: any) => {
        axios.put(`http://localhost:4000/api/notes/${editData._id}/update`, values, { headers })
            .then((response) => {
                setEditForm(false)
